...
#define BH_FUSE_OPT_KEY(t, p, v) { t, offsetof(struct options, p), v }
...
static int bh_fgetattr(const char *path, struct stat *out_st, struct fuse_file_info *fi)
{
  (void)path;
...
static int bh_setxattr(const char * path, const char *name, const char * value, size_t size, int flags)
{
  return_ok_or_errno(setxattr(path, name, value, size, flags));
}

static int bh_getxattr(const char *path, const char *name, char *value, size_t size)
{
...
static int bh_readdir(const char *path, void * buf, fuse_fill_dir_t filler, off_t off, struct fuse_file_info *fi)
{
  (void)path;
...
static int bh_fsyncdir(const char *path, int datasync, struct fuse_file_info *fi)
{
  (void)path;
...
static int bh_read(const char *path, char * buf, size_t buf_size, off_t off, struct fuse_file_info *fi)
{
  (void)path;
  ...
  size_t rsize = ((off + buf_size) > (size_t)st.st_size) ? (st.st_size - off) : buf_size;

  bzero(buf, buf_size);
  return rsize;
}

static int bh_write(const char *path, const char * buf, size_t size, off_t off, struct fuse_file_info *fi)
{
  (void)path;
  (void)buf;
  ...
  if (newsize > oldsize) {
    ...
    errno = 0;
    if (fallocate(fd, FALLOC_FL_PUNCH_HOLE | FALLOC_FL_KEEP_SIZE, 0, newsize) == -1) {
      return -errno;
    }
  }
  return size;
...
static int bh_ftruncate(const char *path, off_t length, struct fuse_file_info *fi)
{
  (void)path;
  ...
  errno = 0;
  if (fallocate(fd, FALLOC_FL_PUNCH_HOLE | FALLOC_FL_KEEP_SIZE, 0, length) == -1) {
    return -errno;
  }
...
static int bh_fallocate(const char * path, int mode, off_t off, off_t len, struct fuse_file_info *fi)
{
  (void)path;
  (void)mode;
